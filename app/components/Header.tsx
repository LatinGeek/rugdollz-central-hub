...
export function Header({ onMenuClick, isOpen = false }: HeaderProps) {
  ...
  const { user, isAuthenticated, connect, disconnect, defaultAvatar, isLoading: authIsLoading } = useAuth();
  const { useTokenBalance } = useWalletService();
  const { balance, isLoading: balanceIsLoading } = useTokenBalance(user?.address, "RUGZ");
